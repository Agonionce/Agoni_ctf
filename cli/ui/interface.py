...
class RichPromptToolkitInterface(UserInterface):
    ...
    def select_mode(self) -> bool:
        ...
        mode_table.add_row("1", "本地自主", "R9.1；仅执行明确许可的本地 Tool")
        mode_table.add_row("2", "人工监督", "默认；风险操作逐个审批")
        self.console.print(mode_table)

        choice = self._prompt_choice("请选择模式 (1/2): ", ["1", "2"], default="2")
        return choice == "1"
    ...
    def approve_tool_call(
        ...
        """Present the R1 action-level approval boundary to the user."""
        detail = (
            f"Tool: {tool_name}\n"
        ...
        border_style = "yellow" if risk_level != "LOW" else "green"
        self.console.print(
            Panel(detail, title="Tool 执行审批", border_style=border_style)
        )
        choice = self._prompt_choice("批准该 ToolCall？(y/n): ", ["y", "n"], default="n")
        return choice == "y"
